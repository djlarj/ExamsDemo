...
// Quiz Data: Generate 10 multiplication problems (1–6 times tables)
const quizData = [];
...
for (let i = 0; i < 10; i++) {
  ...
  while (distractors.size < 3) {
    ...
    if (method === 0) {
      ...
      // Another random multiplication from 1–6 × 0–12
      const altNum1 = Math.floor(Math.random() * 6) + 1;
      const altNum2 = Math.floor(Math.random() * 13);
      wrong = altNum1 * altNum2;
    }

    // Ensure it's valid and not the correct answer
    if (wrong >= 0 && wrong <= 72 && wrong !== correctAnswer) {
